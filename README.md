...
터미널에서 아래 명령을 실행하세요. (`~/.claude.json`에 자동 등록됩니다)

```bash
claude mcp add --scope user igt-design-system node /여기에-클론한-경로/igt-design-system-v4/mcp/server.mjs
```
...
`node_modules` 경로를 사용할 수 있습니다.

```bash
claude mcp add --scope user igt-design-system node /여기에-프로젝트-경로/node_modules/igt-design-system-v4/mcp/server.mjs
```

> **주의:** 과거 안내했던 `~/.claude/mcp.json` 파일은 Claude Code가 읽지 않습니다.  
> 반드시 위 `claude mcp add` 명령을 사용하세요. (등록 후 Claude Code 한 번 재시작)

> **잘 됐는지 확인하는 방법:**  
> 1) 터미널에서 `claude mcp list` → `igt-design-system: ✓ Connected` 표시  
> 2) Claude Code 대화창에 `list_components` 입력 → 컴포넌트 목록이 나오면 연결 성공
