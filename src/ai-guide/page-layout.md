...
.filter-row {
  display: flex;
  align-items: flex-start; /* 라벨 paddingTop으로 수직 정렬 처리 */
  gap: var(--spacing-8);   /* Figma: cont.itemSpacing=8 */
}

/* 인라인 컨트롤 그룹(라디오/체크박스 등) row — 라벨 paddingTop=8 */
...
.filter-row-label {
  ...
  flex-shrink: 0;
  padding-top: 11px;       /* Figma: label paddingTop=11 — h=40 입력 컨트롤(Select/TextField 등)과 수직 정렬 */
  /* Figma text node (6170:2293) 실측: fontSize=15, fontWeight=600, lineHeight=18 */
  /* → --semantic-label-14-semibold */
...
.checkbox-group {
  display: flex;
  flex-wrap: wrap;
  gap: var(--spacing-24);  /* 항목 간 간격 */
  padding: 3px 0;          /* inline frame paddingTop/Bottom 3 */
}

.checkbox-item {
  display: flex;
  align-items: center;
  gap: var(--spacing-4);   /* 컨트롤↔라벨 간격 */
  cursor: pointer;
}
...
RadioBox 행의 컨트롤 영역은 **상하 3px 패딩** + **항목 간 24px gap**을 갖는다. 라디오는 인라인 컨트롤 그룹이므로 row에 `.filter-row--inline` modifier를 부여해 라벨 paddingTop도 8px로 맞춘다.
...
#### 인라인 컨트롤 그룹 row — 라벨/컨텐츠 padding 분기

필터 row는 컨텐츠 타입에 따라 라벨/컨텐츠 padding이 분기된다. Figma node 6170:2288 실측 기준.

| row 컨텐츠 | label paddingTop | content padding Y | 항목 간 gap | 컨트롤↔라벨 gap |
|---|---|---|---|---|
| 라디오 인라인 (단일 선택) | 8 | 3 / 3 | 24 | 4 |
| 체크박스 인라인 wrap (다중 선택) | 8 | 3 / 3 | 24 | 4 |
| Select / TextField / SelectTrigger (h=40 컨트롤) | 11 (기본) | 0 | — | — |
| Block (Select + TagInput 세로 결합) | 11 (기본) | 0 | — | — |

**구조**: 인라인 컨트롤 그룹 row에는 `.filter-row--inline` modifier 부여 → 라벨 padding 8px override. 컨텐츠 Y padding은 그룹 컴포넌트(`.radio-group`, `.checkbox-group`)에 박는다. h=40 단일 컨트롤 row에는 modifier를 붙이지 않는다(기본 11px).

**판단 기준**: 시각 정렬 기준은 의미(단일/다중 선택)가 아니라 컨트롤 크기. 트리거가 h=40 Select 룩이면 multiselect 의미여도 `.filter-row--inline`을 붙이지 않는다.
